...
const TONE_MAP: Record<TagTone, string> = {
  ...
  red: 'border border-warning/10 bg-warning/10 text-warning-80',
  yellow: 'border border-label-yellow-10 bg-label-yellow-10 text-label-yellow-100',
};

export default function CommonTag({ label, tone = 'outline' }: CommonTagProps) {
  return (
    <span
      className={`text-caption inline-flex items-center rounded-full px-2 py-1 font-semibold ${TONE_MAP[tone]}`}
    >
      {label}
    </span>
  );
